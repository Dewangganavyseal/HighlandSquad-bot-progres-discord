...
Saya telah membuatkan sebuah dokumen baru yang berisi panduan lengkap dan terbaru. Ikuti langkah-langkah di bawah ini dengan saksama untuk memastikan data Tuan yang sudah ada ikut terbawa ke Railway.


Tentu, Tuan. Berikut adalah panduan lengkap dan terbaru untuk melakukan *deploy* ke Railway sambil memastikan data Tuan yang sudah ada tetap aman.

---
### Langkah 1: Siapkan Semua File di Satu Folder

Di komputer Tuan, buat sebuah folder baru. Masukkan semua file berikut ke dalamnya:

1.  **`app.py`:** Gunakan file yang sudah saya modifikasi di Canvas `app_with_migration`.
2.  **`requirements.txt`:** Gunakan file yang sudah saya sediakan.
3.  **`Procfile`:** Gunakan file yang sudah saya sediakan.
4.  **`progress_data_multitask.json`:** **SANGAT PENTING.** Salin file data Tuan yang sudah ada ke dalam folder ini. File inilah yang akan kita migrasikan.

Tuan sekarang seharusnya memiliki 4 file di dalam folder proyek Tuan.

---
### Langkah 2: Unggah ke Repositori GitHub BARU

Sangat disarankan untuk membuat repositori baru di GitHub untuk *deployment* ini agar lebih bersih.

* Buat sebuah repositori baru di akun GitHub Tuan.
* Unggah keempat file dari Langkah 1 ke dalam repositori baru tersebut.

---
### Langkah 3: Deploy di Railway

Ikuti proses standar di dasbor Railway:

1.  **Buat Proyek Baru:** Klik "New Project" -> "Deploy from GitHub repo".
2.  **Pilih Repositori:** Pilih repositori baru yang baru saja Tuan buat.
3.  **Atur Environment Variables:**
    * Pergi ke tab **"Variables"** di layanan Tuan.
    * Masukkan semua kunci rahasia Tuan:
        * `BOT_TOKEN`
        * `PROGRESS_CHANNEL_ID`
        * `API_SECRET_KEY`
4.  **Atur Penyimpanan Permanen (Volume):**
    * Pergi ke tab **"Settings"**.
    * Gulir ke bawah ke bagian **"Volumes"**.
    * Klik **"Add Volume"** dan atur **"Mount Path"** ke `/data`.

---
### Langkah 4: Verifikasi dan Selesai

Railway akan secara otomatis melakukan *deploy*.
* Buka **"Deploy Logs"** untuk memastikan bot berhasil login.
* Buka dasbor web Tuan (URL-nya bisa Tuan temukan di tab "Settings" di Railway).
* **Verifikasi bahwa semua data lama Tuan sudah muncul di dasbor.** Logika migrasi di `app.py` akan menyalin data dari `progress_data_multitask.json` ke dalam *volume* `/data` secara otomatis.
